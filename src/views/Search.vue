        ...
        <div class="container pb-5">
            <div class="row py-7 justify-content-center">
                <div class="col-7 d-flex gap-2 border-bottom border-primary py-3 px-0 noto-serif-tc">
                    ...
                    <input id="validationCustom01" type="text" class="form-control py-2" value="" required=""
                        placeholder="輸入關鍵字" />
                </div>
            </div>
            <!-- 隱藏篩選按鈕 -->
            <div class="row pb-3 d-none">
                <div class="col d-flex gap-3">
                    <select id="selectCity" v-model="selectedCity" class="form-select" required>
                        <option selected value="全部地點">全部地點</option>
                        <option v-for="city in CityAll" :key="city.CityName" :value="city.CityName">
                            {{ city.CityName }}
                        </option>
                    </select>
            ...
            <div v-if="selectType === 'searchEvent'" class="row">
                <div v-for="event in eventData" :key="event._id" class="col-6 col-lg-3 mb-3">
                    <div class="card h-100 p-2">
                        <div class="position-relative pt-150per">
                            <p class="py-1 px-2 bg-warning z-1 position-absolute top-0 fs-10 noto-serif-tc">
                                揪團中
                            </p>
                            <img ref="image" class="w-100 h-100 inset-0 object-fit-cover position-absolute"
                                :src="event.eventImageUrl[0]" :alt="event.title" />
                        </div>
                        ...
                        <div class="card-body p-2">
                            ...
                            <p class="card-title noto-serif-tc fw-bold fs-8 mb-0 overflow-hidden card-title-h">
                                {{ event.title }}
                            </p>
                            ...
                            <div style="" class="bg-greyE9 rounded-1 pb-2 mb-2 position-relative">
                                <div class="position-absolute h-100 start-0 bg-blue-light rounded-4" :style="{
                        right: lineStyle(
                            event.currentParticipantsCount,
                            event.maxParticipants
                        ),
            ...
            <div v-else class="row">
                <div v-for="store in storeData" :key="store.user" class="col-6 col-lg-3 mb-3">
                    <div :style="{
                        ...
                        <div
                            ...
                            <img width="50%" height="22.7%" class="rounded-circle object-fit-cover mb-2 border"
                                :src="store.avatar" :alt="store.name" />
